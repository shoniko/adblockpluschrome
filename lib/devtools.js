...
const {MlModel} = require("../adblockpluscore/lib/ml");
...
function getMlInference(request)
{
  let decisionBoundary = 0.9;
  if (request.url)
  {
    let inference = MlModel.predict(request.url);
    return {
      inference,
      decision: inference > decisionBoundary
    };
  }
  return {};
}
...
function addRecord(panel, request, filter)
{
  if (!hasRecord(panel, request, filter))
  {
    panel.port.postMessage({
      ...
      mlinference: getMlInference(request)
    });
